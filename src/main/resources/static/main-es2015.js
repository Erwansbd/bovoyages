...
let DestinationDetailsComponent = class DestinationDetailsComponent {
    constructor(activatedRoute, destinationService) {
...
let DatesVoyageComponent = class DatesVoyageComponent {
    constructor() { }
    ngOnInit() {
    }
};
    ...
    ngOnInit() {
        this.destinationService.getDestinations().subscribe((destinations) => {
            ...
            this.destinations.forEach(destination => {
                ...
                this.destinationService.getImages(destination.id).subscribe((images) => {
                    images.forEach(img => destination.imageUrl = images[0].image);
                });
            });
        });
    }
...
let LoginComponent = class LoginComponent {
    constructor(authService, clientService, router, route) {
        this.authService = authService;
        this.clientService = clientService;
        this.router = router;
        this.route = route;
...
let MesvoyagesComponent = class MesvoyagesComponent {
    constructor(destinationService, activatedRoute) {
        this.destinationService = destinationService;
        this.activatedRoute = activatedRoute;
    }
    ngOnInit() {
        this.activatedRoute.paramMap.subscribe((map) => {
            ...
            this.destinationService.getVoyagesByClient(name).subscribe(voyages => this.voyages = voyages);
        });
    }
};
MesvoyagesComponent.ctorParameters = () => [
    { type: _shared_destinations_service__WEBPACK_IMPORTED_MODULE_2__["DestinationsService"] },
...
let VoyageComponent = class VoyageComponent {
    constructor(destinationService, router, clientService, activatedRoute, httpClient) {
        this.destinationService = destinationService;
        this.router = router;
        this.clientService = clientService;
        this.activatedRoute = activatedRoute;
    ...
    ngOnInit() {
        ...
        this.activatedRoute.paramMap.subscribe((map) => {
            const id = +map.get('id');
            this.destinationService.getDestinationByDatesVoyage(id).subscribe(destination => this.destination = destination);
            this.destinationService.getDatesVoyageById(id).subscribe(datesVoyage => this.datesVoyage = datesVoyage);
    ...
    onSubmit() {
        this.destinationService.submitVoyage(this.form.value).subscribe((data) => {
...
VoyageComponent.ctorParameters = () => [
    { type: _shared_destinations_service__WEBPACK_IMPORTED_MODULE_2__["DestinationsService"] },
...
let AuthService = class AuthService {
    constructor(router, httpClient, clientService) {
        this.router = router;
        this.httpClient = httpClient;
        this.clientService = clientService;
        this.isAuth = false;
    ...
    login(client, cb) {
        ...
        this.httpClient.post(this.url + 'connexion', params).subscribe((status) => {
            if (status) {
    ...
    register(client) {
        const params = new _angular_common_http__WEBPACK_IMPORTED_MODULE_3__["HttpParams"]()
            .set('nom', client.nom)
            .set('password', client.password);
        this.httpClient.post(this.url + 'signup', params).subscribe((status) => {
            if (status) {
                this.router.navigate(['/login']);
            }
            else {
...
let DestinationsService = class DestinationsService {
    ...
    getDestinationByDatesVoyage(id) {
